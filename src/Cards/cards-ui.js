...
function fanCards(cards, self, options) {
    ...
    coords.forEach(function (coord) {
        ...
        card.onmouseover = function () {
            ...
            const isOpposite = options.fanDirection === 'S' || options.fanDirection === 'W';
            const updatedCoord = isOpposite ? 10 : -10;
            self.cardSetTop(card, coord.y + updatedCoord);
        };
...
$(window).on('load', function () {
    ...
    $(".fan:not([data-bind])").each(function () {
        // module.fan($(this), { fanDirection: 'E'});
    });
...
export const doMagic = (id, options = {}) => {
    $(`#${id} .fan:not([data-bind])`).each(function () {
        module.fan($(this), options);
    });
}